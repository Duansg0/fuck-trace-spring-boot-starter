...
import com.talkee.trace.interceptor.PvDigestInterceptor;
...
public class TraceAutoConfiguration extends AbstractConfiguration {
    ...
    @ConditionalOnProperty(prefix="spring.boot.trace.traceSwitch",name = "Dao", havingValue = "true")
    ...
    @ConditionalOnProperty(prefix="spring.boot.trace.traceSwitch",name = "Pv", havingValue = "true")
    public DefaultPointcutAdvisor defaultPointcutAdvisorPv(TraceProperties traceProperties) {
       super.init(traceProperties);
       return InterceptorBuilder.build(new PvDigestInterceptor(), new InterceptorInitInfoModel.Builder().buildAppName(traceProperties.getAppName()).buildExecution(traceProperties.getTracePvExecution()).build());
    }
    ...
    @ConditionalOnProperty(prefix="spring.boot.trace.traceSwitch",name = "Feign", havingValue = "true")
